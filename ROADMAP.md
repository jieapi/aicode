...
#### 2.2 Lint 检查工具
```kotlin
class LintCheckTool @Inject constructor() : AgentTool() {
    override val name = "lint_check"
    override val description = "对代码进行静态检查"
    
    override suspend fun execute(args: Map<String, Any>): ToolResult {
        // 集成 Detekt、Ktlint、ESLint 等
    }
}
```

**任务**：
- [ ] 集成 Detekt for Kotlin
- [ ] 集成 Ktlint for 代码格式
- [ ] 支持自定义规则

#### 2.3 格式化工具
```kotlin
class FormatCodeTool @Inject constructor() : AgentTool() {
...
- [ ] 自定义 API 请求地址 (如支持中转代理、本地模型如 Ollama 等)
...
**下一步行动**: 第二阶段工作基本完成。接下来的重头戏是 4.1 多提供商面板 (让用户能够在 UI 界面中切换并输入密钥)、4.2 代码编辑器集成，以及 2.2 代码检查工具的接入！